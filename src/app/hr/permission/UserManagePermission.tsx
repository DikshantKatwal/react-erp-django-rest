...
const UserPermissionForm: React.FC<UserPermissionFormProps> = () => {
    ...
    const permission = location.state?.user;
    ...
    const [fetchUserPermissionsAPI] = UserService.usePermissionUserMutation();
    ...
    useEffect(() => {
        if (permission && Array.isArray(permission.user_permissions)) {
            setSelectedPermissions(permission.user_permissions);
        }
    }, [permission]);
    ...
    const handleSave = () => {
        fetchUserPermissionsAPI({ user_id: permission.id, permission_id:selectedPermissions}).unwrap().then(() => {

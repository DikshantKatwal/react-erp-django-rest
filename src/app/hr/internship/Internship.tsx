...
const EditInternForm: React.FC<NewEmployeeFormProps> = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const intern = location.state?.intern;
    ...
    useEffect(() => {
        ...
        if (intern) {
            setFormState(prevState => ({
                ...prevState,
                ...intern, // Spread the data into the form state
                email: intern.email, // Set email specifically if needed
                id: intern.id 
            }));
        }
    }, [intern]);
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (intern) {
            update_employee({ id: intern.id, ...payload })
                .unwrap()
                .then(() => {
